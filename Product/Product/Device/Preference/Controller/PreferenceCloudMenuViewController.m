//
//  PreferenceCloudMenuViewController.m
//  Product
//
...
@interface PreferenceCloudMenuViewController ()<UIAlertViewDelegate,UITableViewDelegate,UITableViewDataSource>{
...
@implementation PreferenceCloudMenuViewController
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    PreferenceCloudMenuDetailVC *cloudMenuDetailVC = [[PreferenceCloudMenuDetailVC alloc] init];
...
- (void)requestCloudMenu{
    ...
    NSString *prefenceType=[TonzeHelpTool sharedTonzeHelpTool].prefrenceType;
    if (index==6) {
        if ([prefenceType isEqualToString:@"降压粥"]) {
             urlStr = [NSString stringWithFormat:@"page_num=%ld&page_size=20&type=1&equipment=%ld&tag=3",(long)page,index];
        }else{
         urlStr = [NSString stringWithFormat:@"page_num=%ld&page_size=20&type=1&equipment=%ld&tag=4",(long)page,index];
        }
    }else{
