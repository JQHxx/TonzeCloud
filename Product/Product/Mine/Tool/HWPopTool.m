...
- (void)setCloseButtonType:(ButtonPositionType)closeButtonType {
    ...
    if (closeButtonType == ButtonPositionTypeNone) {
        ...
        }else if (closeButtonType == ButtonPositionTypeBottom){
            closeFrame.origin.x = round((CGRectGetWidth(self.containerView.frame)- CGRectGetWidth(closeFrame))/2);
            closeFrame.origin.y = round(CGRectGetHeight(self.containerView.frame)+10);
...
//            containerView.transform = CGAffineTransformScale(CGAffineTransformIdentity, 0.0, 0.0);
            [UIView animateWithDuration:kTransformPart1AnimationDuration animations:^{
                ...
                [UIView animateWithDuration:.2 animations:^{
                    containerView.alpha = 1;
                    containerView.transform = CGAffineTransformScale(CGAffineTransformIdentity, 1.0, 1.0);
                } completion:^(BOOL finished) {
...
- (void)closeAnimation:(BOOL)animated WithBlcok:(void(^)())complete{
    [self hideAnimated:animated withCompletionBlock:complete];
}

- (void)hideAnimated:(BOOL)animated withCompletionBlock:(void(^)())completion {
    if (!completion) {
        if ([_hwpopDelegate respondsToSelector:@selector(HwpopData)]) {
            [_hwpopDelegate HwpopData];
        }
    }
    if(!animated){
        [self cleanup];
