 ...
 *  原已选中的标签
 */
@property (strong, nonatomic) TTGTextTagLabel * oldLabel;
...
- (void)addObjTags:(NSMutableArray *)tags {
    ...
    for (int index = 0;index < [tags count];index++)
    {
        ...
        {
            TTGTextTagLabel *label = [self newLabelForTagText:obj];
        ...
        else if ([obj isKindOfClass:[NSDictionary class]])
        {
            ...
            TTGTextTagLabel *label = [self newLabelForTagText:dic[@"name"]];
            [label setIndex:[dic[@"index"] integerValue]];
            [_tagLabels addObject:label];
        }
    }
    [_tagCollectionView reload];
...
- (void)setTagAtIndex:(NSUInteger)index selected:(BOOL)selected {
    ...
    self.oldLabel = _tagLabels[index];
...
//        {
//            TTGTextTagLabel * label = (TTGTextTagLabel *)recognizer.view;
//            self.deleteAction(label.text);
//        }
    }
...
- (void)tagCollectionView:(TTGTagCollectionView *)tagCollectionView didSelectTag:(UIView *)tagView atIndex:(NSUInteger)index {
    ...
    if (_enableTagSelection)
    {
        if (self.radioTag) {
            // 清除已选中的样式
            if(self.oldLabel && self.oldLabel.index != index){
                self.oldLabel.selected = NO;
                [self resetStyleForLabel:self.oldLabel];
            }
