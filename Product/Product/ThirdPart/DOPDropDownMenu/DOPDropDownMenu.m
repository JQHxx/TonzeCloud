...
//        CAShapeLayer *indicator = [self createIndicatorWithColor:self.indicatorColor andPosition:CGPointMake((i + 1)*separatorLineInterval - 10, self.frame.size.height / 2)];
        CAShapeLayer *indicator = [self createIndicatorWithColor:self.textColor andPosition:CGPointMake((i + 1)*separatorLineInterval - 20, self.frame.size.height / 2)];
...
//    DYLog(@"bglayer bounds:%@",NSStringFromCGRect(layer.bounds));
//    DYLog(@"bglayer position:%@", NSStringFromCGPoint(position));
...
//    [path closePath];
    ...
    [path moveToPoint:CGPointMake(0, 0)];
    [path addLineToPoint:CGPointMake(9, 8)];
...
- (CAShapeLayer *)createSeparatorLineWithColor:(UIColor *)color andPosition:(CGPoint)point {
    ...
    CGPathRelease(bound);
    layer.position = point;
    //DYLog(@"separator position: %@",NSStringFromCGPoint(point));
    //DYLog(@"separator bounds: %@",NSStringFromCGRect(layer.bounds));
    return layer;
}
...
- (void)menuTapped:(UITapGestureRecognizer *)paramSender {
     ...
     *  处理别的方法，不显示下拉
     */
    if (tapIndex == self.handleTag) {
        
        if (self.dropDownMenuBlock) {
            
            if(_currentSelectedMenudIndex != -1){
                [self backgroundTapped:nil];
            }
            self.dropDownMenuBlock();
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell) {
        ...
        // 选中视图
        UIImageView *selectImg = [[UIImageView alloc]initWithFrame:CGRectMake(kScreenWidth - 40,(60-25)/2 , 25, 25)];
        selectImg.image = [UIImage imageNamed:@"ic_pub_pick"];
        [bg addSubview:selectImg];
        
        cell.selectedBackgroundView = bg;
        
        cell.textLabel.highlightedTextColor = kSystemColor;
    ...
    if (tableView == _leftTableView) {
        ...
        cell.textLabel.textColor = UIColorHex(0x888888);
