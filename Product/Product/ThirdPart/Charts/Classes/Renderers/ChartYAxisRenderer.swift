    ...
    public override func renderLimitLines(context context: CGContext)
    {
        ...
        for i in 0 ..< limitLines.count
        {
            ...
            if (label.characters.count > 0)
            {
                ...
                {
                    let attrs = [NSFontAttributeName: l.valueFont, NSForegroundColorAttributeName: l.valueTextColor]
                    let size = label.sizeWithAttributes(attrs)
                    ChartUtils.drawText(context: context,
                                        text: label,
                                        point: CGPoint(
                                            x: viewPortHandler.contentLeft - 5,
