...
- (instancetype)init {
    if (self = [super init]) {
        ...
        [self setFrame:CGRectMake(0, 0, DeviceWidth-40, self.collectionView.frame.size.height + CollectionViewVerticalMargin * 2)];
    }
    return self;
}
...
- (void)setupCollectionView {
    CGFloat itemWidth = ((DeviceWidth-40) - CollectionViewHorizonMargin * 2) / 7;
    ...
    CGRect collectionViewFrame = CGRectMake(CollectionViewHorizonMargin, CollectionViewVerticalMargin, (DeviceWidth-40) - CollectionViewHorizonMargin * 2, itemHeight * 6);
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row < firstWeekday) {    // 小于这个月的第一天
        ...
        if (day == [[NSCalendar currentCalendar] component:NSCalendarUnitDay fromDate:self.date]) {
            cell.backgroundColor = kSystemColor;
