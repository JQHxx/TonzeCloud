    ...
    if (_backGroundView == nil) {
        ...
        if (_showtype == ShowTypeIsShareStyle) {
            if (_shareBtnImgArray.count<5) {
                _backGroundView.frame = CGRectMake(7, ActionSheetH, ActionSheetW-14, 64+(_protext.length==0?0:45)+76+34);
            }else
            {
                NSInteger index;
                ...
                _backGroundView.frame = CGRectMake(7, ActionSheetH, ActionSheetW-14, 64+(_protext.length==0?0:45)+76*index+34);
            }
        }
        else
        {
...
- (UIView *)topsheetView
{
    if (_topsheetView == nil) {
        ...
        _topsheetView.backgroundColor = [UIColor bgColor_Gray];
...
- (UILabel *)proL
{
    if (_proL == nil) {
        ...
        _proL.text = @"分享到";
        _proL.textColor = [UIColor darkGrayColor];
        _proL.backgroundColor = [UIColor bgColor_Gray];
